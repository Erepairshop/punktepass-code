...
(function(){
    var AJAX="' . esc_url($ajax_url) . '",
        ...
        STORE_ADDRESS="' . esc_js($store->repair_company_address ?: implode(', ', array_filter([$store->address ?? '', trim(($store->plz ?? '') . ' ' . ($store->city ?? ''))]))) . '",
        STORE_PHONE="' . esc_js($store->repair_company_phone ?: ($store->phone ?? '')) . '",
        STORE_EMAIL="' . esc_js($store->repair_company_email ?: ($store->email ?? '')) . '",
