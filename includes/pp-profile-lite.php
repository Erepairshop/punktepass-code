...
if (!class_exists('PPV_Profile_Lite_i18n')) {
    ...
    class PPV_Profile_Lite_i18n {
        ...
        public static function hooks() {
            ...
            add_action('wp_ajax_ppv_change_email', [__CLASS__, 'ajax_change_email']); // 📧 Email change
            add_action('wp_ajax_nopriv_ppv_change_email', [__CLASS__, 'ajax_change_email']); // ✅ PPV session auth
            add_action('wp_ajax_ppv_change_password', [__CLASS__, 'ajax_change_password']); // 🔑 Password change
            add_action('wp_ajax_nopriv_ppv_change_password', [__CLASS__, 'ajax_change_password']); // ✅ PPV session auth
        }
...
<?php echo esc_textarea($store->maintenance_message ?? ''); ?>
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    // ============================================================
                    const emailBtn = document.getElementById('ppv-change-email-btn');
                    if (emailBtn) {
                        emailBtn.addEventListener('click', function() {
                            ...
                            .then(data => {
                                if (data.success) {
                                    ...
                                    document.getElementById('new_email_confirm').value = '';
                                    // Update display
                                    const currentEmailDisplay = document.querySelector('.ppv-help strong');
                                    if (currentEmailDisplay) {
                                        currentEmailDisplay.textContent = data.data.new_email;
                                    }
                                } else {
                    ...
                    // ============================================================
                    const passwordBtn = document.getElementById('ppv-change-password-btn');
                    if (passwordBtn) {
...
wp_send_json_error(['msg' => 'A cím nem található! Próbáld meg máshogyan írni (pl. teljes utcanévvel).']);
        ...
        public static function ajax_change_email() {
            ...
            $exists = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->prefix}ppv_stores WHERE email = %s AND id != %d",
                $new_email,
                $store_id
            ));
        ...
        public static function ajax_change_password() {
            ...
            $store = $wpdb->get_row($wpdb->prepare(
                "SELECT password FROM {$wpdb->prefix}ppv_stores WHERE id = %d",
                $store_id
            ));
