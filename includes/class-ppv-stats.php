...
class PPV_Stats {
    ...
    public static function enqueue_assets() {
        ...
        // 🏪 FILIALE SUPPORT: Get parent ID and all filialen
        $parent_id = class_exists('PPV_Filiale') ? PPV_Filiale::get_parent_id($store_id) : $store_id;
        ...
        $filiale_ids = array_map(function($f) { return intval($f->id); }, $filialen);
        ...
        $data = [
            ...
            'filiale_ids' => $filiale_ids,
            'lang' => $lang,
            'translations' => $translations,
            'debug' => defined('WP_DEBUG') && WP_DEBUG
        ];

        error_log("📊 [Stats] JS Data: store_id=" . $data['store_id'] . ", parent_id=" . $data['parent_id'] . ", filialen=" . count($filiale_ids));
        wp_add_inline_script('ppv-stats', "window.ppvStats = " . wp_json_encode($data) . ";", 'before');
    }

    // ========================================
    // 🏪 HELPER: Get store IDs for filiale filter
    // ========================================
    private static function get_store_ids_for_filiale($req, $store_id) {
        global $wpdb;

        $filiale_param = sanitize_text_field($req->get_param('filiale') ?? '');
        $parent_id = class_exists('PPV_Filiale') ? PPV_Filiale::get_parent_id($store_id) : $store_id;

        if ($filiale_param === 'all' || empty($filiale_param)) {
            $filialen = $wpdb->get_col($wpdb->prepare(
                "SELECT id FROM {$wpdb->prefix}ppv_stores WHERE id=%d OR parent_store_id=%d",
                $parent_id, $parent_id
            ));
            return array_map('intval', $filialen);
        }

        return [intval($filiale_param)];
    }
    ...
    public static function rest_stats($req) {
        ...
        // 🏪 FILIALE SUPPORT
        $store_ids = self::get_store_ids_for_filiale($req, $store_id);
        $store_ids_placeholder = implode(',', array_fill(0, count($store_ids), '%d'));
        ...
        $daily = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $table_points WHERE store_id IN ($store_ids_placeholder) AND DATE(created)=%s",
        ...
        $all_time = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $table_points WHERE store_id IN ($store_ids_placeholder)",
            ...$store_ids
        ));
        $unique = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(DISTINCT user_id) FROM $table_points WHERE store_id IN ($store_ids_placeholder)",
            ...$store_ids
        ));
        ...
        $top5 = $wpdb->get_results($wpdb->prepare("
            ...
            WHERE p.store_id IN ($store_ids_placeholder)
            GROUP BY p.user_id
        ...
        // Rewards Stats (with IN clause for multiple stores)
        $rewards_total = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $table_redeemed WHERE store_id IN ($store_ids_placeholder)",
            ...$store_ids
        ));
        $rewards_approved = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $table_redeemed WHERE store_id IN ($store_ids_placeholder) AND status IN ('approved', 'bestätigt')",
            ...$store_ids
        ));
        $rewards_pending = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $table_redeemed WHERE store_id IN ($store_ids_placeholder) AND status IN ('pending', 'offen')",
            ...$store_ids
        ));
        $rewards_spent = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT SUM(points_spent) FROM $table_redeemed WHERE store_id IN ($store_ids_placeholder) AND status IN ('approved', 'bestätigt')",
            ...$store_ids
        )) ?? 0;

        // Peak Hours (with IN clause for multiple stores)
        $peak_hours = $wpdb->get_results($wpdb->prepare("
            SELECT HOUR(created) as hour, COUNT(*) as count
            FROM $table_points
            WHERE store_id IN ($store_ids_placeholder) AND DATE(created)=%s
    ...
    public static function rest_trend($req) {
        ...
        $current_week = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $table WHERE store_id IN ($store_ids_placeholder) AND DATE(created) BETWEEN %s AND %s",
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            $count = (int) $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM $table WHERE store_id IN ($store_ids_placeholder) AND DATE(created)=%s",
                ...array_merge($store_ids, [$date])
            ));
            $daily_week[] = [
                'date' => $date,
    ...
    public static function rest_spending($req) {
        ...
        $daily_spending = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT SUM(points_spent) FROM $table_redeemed WHERE store_id IN ($store_ids_placeholder) AND DATE(redeemed_at)=%s AND status IN ('approved', 'bestätigt')",
            ...array_merge($store_ids, [$today])
        )) ?? 0;
        ...
        $avg_reward = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT AVG(points_spent) FROM $table_redeemed WHERE store_id IN ($store_ids_placeholder) AND status IN ('approved', 'bestätigt')",
            ...$store_ids
        )) ?? 0;
        ...
        $top_rewards = $wpdb->get_results($wpdb->prepare(
            "SELECT reward_id, SUM(points_spent) as total, COUNT(*) as count
             FROM $table_redeemed
             WHERE store_id IN ($store_ids_placeholder) AND status IN ('approved', 'bestätigt')
             GROUP BY reward_id
             ORDER BY total DESC
             LIMIT 5",
            ...$store_ids
        ...
        $pending = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT SUM(points_spent) FROM $table_redeemed WHERE store_id IN ($store_ids_placeholder) AND status IN ('pending', 'offen')",
            ...$store_ids
        )) ?? 0;

        $rejected = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT SUM(points_spent) FROM $table_redeemed WHERE store_id IN ($store_ids_placeholder) AND status IN ('rejected', 'abgelehnt')",
            ...$store_ids
        )) ?? 0;
    ...
    public static function rest_conversion($req) {
        ...
        $redeemed_users = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(DISTINCT user_id) FROM $table_redeemed WHERE store_id IN ($store_ids_placeholder)",
            ...$store_ids
        ));
        ...
        $avg_points_per_user = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT AVG(total) FROM (
                ...
                FROM $table_points
                WHERE store_id IN ($store_ids_placeholder)
                GROUP BY user_id
            ) t",
            ...$store_ids
        )) ?? 0;
        ...
        $avg_redemptions_per_user = $redeemed_users > 0
            ? (int) $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) / %d FROM $table_redeemed WHERE store_id IN ($store_ids_placeholder)",
    ...
    public static function render_stats_dashboard() {
        ...
        // 🏪 FILIALE SUPPORT: Get all filialen for this parent store
        $parent_id = class_exists('PPV_Filiale') ? PPV_Filiale::get_parent_id($store_id) : $store_id;
        $filialen = $wpdb->get_results($wpdb->prepare("
        ...
        <div class="ppv-stats-wrapper">
            ...
            <!-- 🏪 FILIALE SELECTOR -->
            <div class="ppv-filiale-selector glass-section" style="margin-bottom: 20px; padding: 16px 20px; background: #f8fafc; border: 2px solid #e2e8f0; border-radius: 12px;">
                <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap;">
                    <label for="ppv-stats-filiale" style="font-weight: 600; margin: 0; color: #334155;">
                        🏢 <?php echo esc_html($T['select_filiale'] ?? 'Filiale auswählen'); ?>:
                    </label>
                    <select id="ppv-stats-filiale" style="flex: 1; min-width: 200px; padding: 10px 14px; border-radius: 10px; border: 2px solid #e2e8f0; background: #fff; color: #1e293b; font-size: 14px; cursor: pointer;">
                        <option value="all" style="font-weight: bold; color: #10b981;">📊 <?php echo esc_html($T['all_filialen'] ?? 'Összes filiale'); ?></option>
                        <?php foreach ($filialen as $filiale): ?>
                            <option value="<?php echo esc_attr($filiale->id); ?>" <?php selected($filiale->id, $store_id); ?>>
                                <?php
                                echo esc_html($filiale->name);
                                if ($filiale->id == $parent_id && (empty($filiale->parent_store_id) || $filiale->parent_store_id === null)) {
                                    echo ' (' . esc_html($T['main_location'] ?? 'Főüzlet') . ')';
                                }
