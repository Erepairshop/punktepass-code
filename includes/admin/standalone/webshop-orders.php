...
final class PPV_Standalone_Webshop_Orders {
    ...
    public static function install_schema() {
        ...
        dbDelta("CREATE TABLE {$table} (
            ...
            shipping_changed_at datetime NULL,
    ...
    public static function import_snapshot() {
        ...
        foreach ($payload['orders'] as $order) {
            ...
            $data = [
                ...
                'shipping_changed_at' => self::nullable_mysql_time($order['shippingChangedAt'] ?? null),
                'ship_name' => $name,
            ...
            if ($existing && $data['shipping_changed_at'] && $data['shipping_changed_at'] !== $existing->shipping_changed_at) {
                $data['packed'] = 0;
                $data['packed_at'] = null;
            }
            $ok = $existing
    ...
    private static function render_orders($management = false) {
        ...
        if ($search !== '') {
            ...
            $where[] = '(order_number LIKE %s OR ship_name LIKE %s OR ship_email LIKE %s OR ship_city LIKE %s OR items_json LIKE %s)';
        ...
        <?php foreach ($orders as $order):
            ...
            $address_change_needs_review = !empty($order->shipping_changed_at)
                && (empty($order->packed_at) || strtotime($order->shipping_changed_at) > strtotime($order->packed_at));
        ?>
            <article class="shop-order <?php echo $order->packed ? 'is-packed' : ''; ?> <?php echo $closed ? 'is-closed' : ''; ?>" data-order-id="<?php echo (int)$order->order_id; ?>">
                ...
                <div class="order-body">
                    <section class="customer">
                        ...
                        <?php if ($management && $order->ship_email): ?><div class="contact"><i class="ri-mail-line"></i> <a href="mailto:<?php echo esc_attr($order->ship_email); ?>"><?php echo esc_html($order->ship_email); ?></a></div><?php endif; ?>
                    </section>
                    <section class="items">
                ...
                <?php if ($address_change_needs_review): ?><div class="address-change-alert"><i class="ri-alert-line"></i><strong>A vásárló módosította a szállítási címet.</strong> Ellenőrizd újra a csomagot és a címkét.</div><?php endif; ?>
