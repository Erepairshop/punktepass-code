/**
 * PunktePass - Signup JavaScript v2
 * Password Strength + Validation + Google OAuth
...
(function($) {
    ...
    function initPasswordToggle() {
        $('.ppv-password-toggle').on('click', function() {
            const $btn = $(this);
            const $input = $btn.closest('.su-input-wrap').find('input');
    ...
    function initPasswordGenerator() {
        $('#ppv-generate-password').on('click', function() {
            ...
            setTimeout(() => { $btn.css({ background: '' }); }, 400);
        });
    }
    ...
    function initPasswordStrength() {
        ...
        function resetRequirements() {
            [$reqLength, $reqUpper, $reqNumber, $reqSpecial].forEach(function($el) {
                $el.removeClass('met unmet');
                $el.find('i').attr('class', 'ri-checkbox-blank-circle-line');
            });
        }
        ...
        function updateRequirement($el, met) {
            if (met) {
                ...
                $el.find('i').attr('class', 'ri-checkbox-circle-fill');
            } else {
                $el.addClass('unmet').removeClass('met');
                $el.find('i').attr('class', 'ri-close-circle-line');
            }
        }
    }
    ...
    function showFieldError($field, message) {
        $field.css({ 'border-color': '#EF4444', 'background': '#FEE2E2' });
        if (!$field.next('.ppv-field-error').length) {
            $field.after('<span class="ppv-field-error">' + message + '</span>');
        }
    }
    ...
    function openGoogleButtonFallback() {
        if (googleButtonRendered) {
            var renderedBtn = document.querySelector('#ppv-google-rendered-btn div[role="button"]');
            if (renderedBtn) renderedBtn.click();
            return;
        }

        var $btn = $('#ppv-google-signup-btn');
        ...
        setTimeout(function() {
            var renderedBtn = container.querySelector('div[role="button"]');
            if (renderedBtn) renderedBtn.click();
        }, 100);
    }
    ...
    function resetGoogleButton($btn) {
        $btn.prop('disabled', false).html(
            ...
            '</svg><span>' + t('signup_google_btn') + '</span>'
        );
    }
    ...
    function initFormSubmit() {
        $('#ppv-signup-form').on('submit', function(e) {
            ...
            var $form = $(this);
            var $btn = $('#ppv-submit-btn');
            var $btnText = $btn.find('.ppv-btn-text');
            var $btnLoader = $btn.find('.ppv-btn-loader');

            var email = $('#ppv-email').val().trim();
            var password = $('#ppv-password').val();
            var passwordConfirm = $('#ppv-password-confirm').val();
            var terms = $('#ppv-terms').is(':checked');
            var privacy = $('#ppv-privacy').is(':checked');
            var userType = $('#ppv-user-type').val() || 'user';
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.success) {
                        ...
                        setTimeout(function() { $form.css('animation', ''); }, 500);
                    }
                },
                error: function() {
                    showAlert(t('error_connection'), 'error');
